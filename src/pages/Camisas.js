      ...
      <StyledProductsWrapper>
        <StyledFilters style={hideFilters ? {display: 'none'} : {display: 'grid'}}>
          ...
          <div style={{marginBottom: "10px"}}>
            ...
            <StyledULFilters>
              <StyledLIFilters><input type={'radio'} name='tipos' value='Plana' onClick={(e) => setFilterType(e.target.value)}/> Plana</StyledLIFilters>
              <StyledLIFilters><input type={'radio'} name='tipos' value='Rayas' onClick={(e) => setFilterType(e.target.value)}/> Rayas</StyledLIFilters>
              <StyledLIFilters><input type={'radio'} name='tipos' value='Puntos' onClick={(e) => setFilterType(e.target.value)}/> Puntos</StyledLIFilters>
              <StyledLIFilters><input type={'radio'} name='tipos' value='Diseñada' onClick={(e) => setFilterType(e.target.value)}/> Diseñada</StyledLIFilters>
            </StyledULFilters>
          </div>
        ...
        <StyledProducts>
          ...
          Object.entries(productItem).map(([indexNumber, product]) => {
            if (product.section === 'Camisas' && product.name.includes(search) && product.color.includes(filterColor) && product.type.includes(filterType)) {
              return (
