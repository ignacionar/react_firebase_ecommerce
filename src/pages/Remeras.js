      ...
      <StyledProductsWrapper>
        <StyledFilters style={hideFilters ? {display: 'none'} : {display: 'grid'}}>
          ...
          <div style={{marginBottom: "10px"}}>
            ...
            <StyledULFilters>
              <StyledLIFilters><input type={'radio'} name='tipos' value='Corta' onClick={(e) => setFilterType(e.target.value)}/> Corta</StyledLIFilters>
              <StyledLIFilters><input type={'radio'} name='tipos' value='Larga' onClick={(e) => setFilterType(e.target.value)}/> Larga</StyledLIFilters>
              <StyledLIFilters><input type={'radio'} name='tipos' value='Musculosa' onClick={(e) => setFilterType(e.target.value)}/> Musculosa</StyledLIFilters>
              <StyledLIFilters><input type={'radio'} name='tipos' value='Apretada' onClick={(e) => setFilterType(e.target.value)}/> Apretada</StyledLIFilters>
            </StyledULFilters>
          </div>
        ...
        <StyledProducts>
          ...
          Object.entries(productItem).map(([indexNumber, product]) => {
            if (product.section === 'Remeras' && product.name.includes(search) && product.color.includes(filterColor) && product.type.includes(filterType)) {
              return (
