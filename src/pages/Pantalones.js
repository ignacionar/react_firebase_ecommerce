      ...
      <StyledProductsWrapper>
        <StyledFilters style={hideFilters ? {display: 'none'} : {display: 'grid'}}>
          ...
          <div style={{marginBottom: "10px"}}>
            ...
            <StyledULFilters>
              <StyledLIFilters><input type={'radio'} name='tipos' value='Jeans' onClick={(e) => setFilterType(e.target.value)}/> Jeans</StyledLIFilters>
              <StyledLIFilters><input type={'radio'} name='tipos' value='Cargo' onClick={(e) => setFilterType(e.target.value)}/> Cargo</StyledLIFilters>
              <StyledLIFilters><input type={'radio'} name='tipos' value='Joggers' onClick={(e) => setFilterType(e.target.value)}/> Joggers</StyledLIFilters>
              <StyledLIFilters><input type={'radio'} name='tipos' value='Shorts' onClick={(e) => setFilterType(e.target.value)}/> Shorts</StyledLIFilters>
            </StyledULFilters>
          </div>
        ...
        <StyledProducts> 
          ...
          Object.entries(productItem).map(([indexNumber, product]) => {
            if (product.section === 'Pantalones' && product.name.includes(search) && product.color.includes(filterColor) && product.type.includes(filterType)) {
              return (
